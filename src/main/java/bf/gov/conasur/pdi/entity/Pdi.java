...
public class Pdi {
    ...
    @Enumerated(EnumType.STRING)
    @Column(name = "statut_courant", nullable = false, length = 30)
    private StatutPDI statutCourant;

    @Builder.Default
    @Column(name = "date_enrolement", nullable = false)
    private LocalDate dateEnrolement = LocalDate.now();
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "id_site_courant", nullable = false)
    private SiteAccueil siteCourant;
}
